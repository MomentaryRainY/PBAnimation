...
public class Rigid_Bunny_by_Shape_Matching : MonoBehaviour
{
    ...
    {
        Vector3 plane1 = new Vector3(0, 0.01f, 0);
        Vector3 plane1N = new Vector3(0, 1, 0);
        Vector3 plane2 = new Vector3(2, 0, 0);
        Vector3 plane2N = new Vector3(-1, 0, 0);

        Vector3[] Y = new Vector3[V.Length];
        for (int i = 0; i < V.Length; i++)
        {

            if (Vector3.Dot(X[i] - plane1, plane1N) < 0)
            {
                Vector3 Vn = Vector3.Dot(V[i], plane1N) * plane1N;
                Volocity(i, Vn);
            }
            else if (Vector3.Dot(X[i] - plane2, plane2N) < 0)
            {
                Vector3 Vn = Vector3.Dot(V[i], plane2N) * plane2N;
                Volocity(i, Vn);
            }
        }
    ...
    void Volocity(int i, Vector3 Vn)
    {
        Vector3 Vt = V[i] - Vn;
    ...
    void Update()
    {
        if (Input.GetKey("r"))
        {
            ...
            Update_Mesh(new Vector3(0, 0.6f, 0), Matrix4x4.Rotate(Quaternion.Euler(new Vector3(50, 0, 0))), 0);
            launched = false;
        }
        if (Input.GetKey("l"))
        {
        ...
        if (launched)
        {
            ...
            for (int i = 0; i < Y.Length; i++)
            {
                A = MatrixAdd(A, MatrixMul((Y[i] - c), Q[i]));
            }
            A[3, 3] = 1.0f;
            Matrix4x4 R = Get_Rotation(A * QQt.transpose);
    ...
    Matrix4x4 MatrixMul(Vector3 a, Vector3 b)
    {
        Matrix4x4 c = Matrix4x4.zero;
        c[3, 3] = 1.0f;
        for (int i = 0; i < 3; i++)
